# 1 Sampling

**Course day 2  
Tuesday, 22.09.2026**

---
### Materials and Equipment

**Consumables:**

* Plastic bag for transporting fish

**Equipment:**

* Camera
* Sample labels
---

Good morning!

🐟 Today, you will go together with your fellow participants, the teachers, and the assistants to the local fish market in Bissau. The goal of the course is to select a broad variety of species from the mostly marine fish on offer. Please talk to your fellow course participants and teachers if you find a fish of interest, so that we do not buy too many specimens of the same species.

<img src="https://images.squarespace-cdn.com/content/v1/5d402069d36563000151fa5b/1643793579051-SO73AAT676YH2QKZDBJH/10+CABDALI_BISSAU_MARCHE_37.jpg?format=1500w" /> ***Figure 1:** Fish market in Bissau. © Carmen Yasmine Abd Ali*

> [!IMPORTANT]
> For the course, we will use genetic primers specifically developed for vertebrates. Please choose a fish species and do not select crustaceans, mollusks, nor echinoderms. You can choose either a chondrichthyan (cartilaginous fish) or an osteichthyan (bony fish). The fish must be untreated, i.e. neither cooked nor salted.

➤ **1.1** 📝 Note the following information for your chosen fish specimen (information in bold is mandatory):
> - **Name of the fish in Guinea-Bissau Creole and/or Portuguese, as given by the seller:**
> - Location of capture (if known by the seller; optional):
> - Trivial name that you know for this species (optional):
> - Scientific name based on morphology (if you know; optional):
> - **Name of collector (you):**
> - **Collection date:**
> - **Collection location:**

> [!NOTE]
> Proper documentation of information about a sample during collection is essential for scientific research. Molecular data cannot be used effectively in scientific research if the information about the sample was not recorded or has been lost. In that case, your laboratory work may have been a waste of time. Always think before collecting a sample about what information might become important for your research, and record this information along with the sample. When in doubt, collect more information than you expect to need. Take the *Collection date* and *Collection location*, for example. Of course, we collected all the fish for the course on the same day and at the same fish market in Bissau. Nevertheless, the date and location are essential standard information for any kind of scientific collection.

🏠 We are back in our lab!  
Everyone has their own fish and the assigned collection data.

➤ **1.2** 📷 Take a picture of the entire specimen before taking a tissue sample (see Figure 2). Place the specimen number next to the fish in the photo. This number will remain associated with your specimen throughout the course, allowing us to link the fish we sampled to the genetic data generated at the end of the course. If you do not have a mobile phone or camera to take a picture, please ask a teacher for help.

> <img src="https://static.inaturalist.org/photos/622863606/original.jpg" /> ***Figure 2:**
> This is an orfe (*Leuciscus idus*), a freshwater fish caught during ice fishing in the Baltic sea close to Oulu, Finland. Based on the photo a morphological species identification is possible. The whole body is visible.*

➤➤➤ The next step is the **[2. DNA Extraction](https://github.com/biodiversitygenomicsoulu/Free-Course-on-Portable-Molecular-Biology-for-Biodiversity-Monitoring/blob/main/2_DNA_Extraction_%F0%9F%A7%AC_English_Version.md)**.
